...
// Capture toute sortie parasite (warnings, notices PHP) avant les headers JSON
ob_start();

// Supprime l'affichage des erreurs PHP dans la réponse (elles iraient dans les logs)
ini_set('display_errors', '0');
error_reporting(E_ALL);

// Le fichier config.php est à la racine du projet (un niveau au-dessus de Controller/)
...
class ReclamationController
{
    ...
    {
        // Vider tout ce qui aurait pu être mis en buffer (warnings PHP, etc.)
        if (ob_get_level() > 0) {
            ob_end_clean();
        }
    ...
    public function showReclamation(): void
    {
        ...
        require __DIR__ . '/../View/backoffice-formation/listreclamtion.php';
    }
...
// N'exécuter handleRequest() que si ce fichier est appelé directement (pas via include)
