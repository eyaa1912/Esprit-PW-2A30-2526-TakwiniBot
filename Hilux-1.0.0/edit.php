...
$selectStmt = $pdo->prepare('SELECT * FROM entretien WHERE id_entretien = :id');
...
$data = [
    ...
    'type_entretien' => (string) $entretien['type_entretien'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        try {
            ...
            $errors['global'] = 'Impossible de modifier l’entretien. Vérifiez les données puis réessayez.';
        }
    }
}
?>
...
<main class="container py-4">
    <section class="top-bar mb-4">
        <div>
            <h1 class="mb-1">Modifier l’entretien #<?= $id ?></h1>
            <p class="text-muted mb-0">Mise à jour des informations du candidat</p>
        </div>
        <a href="show.php?id=<?= $id ?>" class="btn btn-outline-secondary">Retour au détail</a>
    </section>
        ...
        <div class="col-lg-4">
            <div class="avatar-card">
                ...
                <p id="avatar-help" class="avatar-bubble">
                    Mettons à jour les informations pour garder un suivi inclusif et précis.
                </p>
            </div>
        </div>
                ...
                <div class="card-body">
                    <form method="post" class="needs-validation" novalidate data-avatar-help="true" data-avatar-help-target="avatar-help">
                        ...
                        <div class="mt-4 d-flex gap-2">
                            <button type="submit" class="btn btn-takwini">Mettre à jour</button>
                            <a href="show.php?id=<?= $id ?>" class="btn btn-outline-secondary">Annuler</a>
                        </div>
                    </form>
                </div>
            </div>
