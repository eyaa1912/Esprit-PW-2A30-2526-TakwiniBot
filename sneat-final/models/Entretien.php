...
class Entretien {
    ...
    public function getAllEntretiens($filters = []) {
        ...
        if (!empty($filters['sort']) && $filters['sort'] === 'candidat_asc') {
            $sql .= " ORDER BY e.nom_candidat ASC";
        } elseif (!empty($filters['sort']) && $filters['sort'] === 'candidat_desc') {
            $sql .= " ORDER BY e.nom_candidat DESC";
        } else {
    ...
    /**
     * Statistiques par poste (pour graphique)
     */
    public function getStatsByPoste() {
        $sql = "SELECT COALESCE(NULLIF(TRIM(poste_cible), ''), 'Non défini') AS poste_cible, COUNT(*) AS total
                FROM entretien
                GROUP BY COALESCE(NULLIF(TRIM(poste_cible), ''), 'Non défini')
                ORDER BY total DESC";
    ...
    public function getTypesEntretien() {
        $sql = "SELECT id, libelle FROM type_entretien ORDER BY libelle";
        $stmt = $this->pdo->query($sql);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    public function createEntretien($data) {
        $sql = "INSERT INTO entretien (
                    ...
                    metier_suggere, score_rse, remarques, statut
                ) VALUES (
                    :nom_candidat, :email_candidat, :genre, :type_handicap, :amenagements,
                    :type_entretien_id, :date_entretien, :heure_entretien, :poste_cible,
                    :metier_suggere, :score_rse, :remarques, :statut
